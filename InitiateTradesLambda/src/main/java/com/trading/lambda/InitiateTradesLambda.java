...
public class InitiateTradesLambda implements RequestHandler<Map<String, Object>, String> {
    ...
    public String handleRequest(Map<String, Object> input, Context context) {
        try {
            ...
            for (Map.Entry<String, Double> entry : tickerPositionSizes.entrySet()) {
                ...
                try {
                    context.getLogger().log("DEBUG: Starting option contract selection for " + ticker);
                    ...
                    if (optionContracts == null || optionContracts.isEmpty()) {
                        context.getLogger().log("DEBUG: No option contracts found for " + ticker + " - skipping");
                        continue;
                    }

                    String nearSymbol = (String) optionContracts.get("near_symbol");
                    String farSymbol = (String) optionContracts.get("far_symbol");
                    context.getLogger().log("DEBUG: Selected contracts for " + ticker + " - Near: " + nearSymbol + ", Far: " + farSymbol);
                    
                    double debit = calculateDebit(nearSymbol, farSymbol, credentials);
                    if (debit <= 0) {
                        context.getLogger().log("DEBUG: Invalid debit calculated for " + ticker + ": " + debit + " - skipping");
                        continue;
                    }

                    int quantity = calculateQuantity(debit, totalDebitTarget);
                    if (quantity < 1) {
                        context.getLogger().log("DEBUG: Invalid quantity calculated for " + ticker + ": " + quantity + " - skipping");
                        continue;
                    }
                    
                    context.getLogger().log("DEBUG: Proceeding with order submission for " + ticker + " - Debit: " + debit + ", Quantity: " + quantity);
    ...
    private String extractScanDate(Map<String, Object> input) {
        ...
        if (scanDate == null || scanDate.contains("<aws.events.rule.schedule-time>")) {
            // No scanDate provided or EventBridge template not resolved, use current date
     ...
     * For calendar spreads: Net Debit = Far Ask - Near Bid
    ...
    public double calculateDebit(String nearSymbol, String farSymbol, AlpacaCredentials credentials) {
        return TradingCommonUtils.executeWithErrorHandling("calculating debit", () -> {
            ...
            if (nearQuote == null || farQuote == null) {
                System.out.println("DEBUG: Missing quote for near: " + nearSymbol + " or far: " + farSymbol);
                return 0.0;
            }
            ...
            // Debug logging
            System.out.println("DEBUG: Debit calculation for " + nearSymbol + " vs " + farSymbol);
            System.out.println("DEBUG: Near bid: " + nearBid + ", Far ask: " + farAsk);
            System.out.println("DEBUG: Raw calculation (farAsk - nearBid): " + (farAsk - nearBid));
            
            double debit = Math.max(0.0, farAsk - nearBid);
            System.out.println("DEBUG: Final debit after Math.max: " + debit);
