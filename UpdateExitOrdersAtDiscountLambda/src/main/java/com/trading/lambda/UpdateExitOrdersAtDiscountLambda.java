...
/**
 * AWS Lambda function for updating exit orders with new limit prices set to 3% below market.
 * Simple single-purpose lambda with no time-based logic - EventBridge handles timing.
 * Processes all exit orders and updates them to 3% below current market price.
 */
public class UpdateExitOrdersAtDiscountLambda implements RequestHandler<Map<String, Object>, String> {
    
    private static final String ALPACA_SECRET = System.getenv("ALPACA_SECRET_NAME");
    
    public UpdateExitOrdersAtDiscountLambda() {
        // No initialization needed - using shared utilities
    }
    
    // Constructor for testing
    public UpdateExitOrdersAtDiscountLambda(Object testParam) {
        // No initialization needed - using shared utilities
    }
    ...
    public String handleRequest(Map<String, Object> input, Context context) {
        try {
            context.getLogger().log("Starting UpdateExitOrdersAtDiscountLambda execution");
            ...
            for (Map<String, Object> order : exitOrders) {
                try {
                    ...
                    // Calculate current spread price and set to 3% below market
                    double currentSpreadPrice = TradingCommonUtils.calculateCurrentSpreadPrice(order, credentials);
                    if (currentSpreadPrice > 0) {
                        double exitDiscount = TradingCommonUtils.getExitDiscount();
                        double newLimitPrice = currentSpreadPrice * exitDiscount;
                        context.getLogger().log("Updating exit order " + orderId + " to 3% below market: " + newLimitPrice);
            ...
            context.getLogger().log("UpdateExitOrdersAtDiscountLambda completed: " + result);
            return result;
            
        } catch (Exception e) {
            return TradingErrorHandler.handleError(e, context, "UpdateExitOrdersAtDiscountLambda");
        }
    }
}
